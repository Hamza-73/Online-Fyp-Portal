...
export default function EditSupervisorProfile() {
  ...
  const { editSupervisorProfile, getSupervisorProfile } =
    useContext(SupervisorContext); // Access edit function from context
  ...
  const isSaveDisabled = Object.keys(originalData).every(
    (key) =>
      ...
      editedSupervisor[key].toString().trim().toLowerCase()
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div className="flex items-center space-x-2">
                ...
                <input
                  ...
                  name="isCommittee"
                  checked={
                    editedSupervisor.isCommittee === "true" ||
                    editedSupervisor.isCommittee === true
                  }
                  onChange={(e) =>
                    setEditedSupervisor((prev) => ({
                      ...prev,
                      isCommittee: e.target.checked,
                    }))
                  }
                  disabled={!isEditing}
                  className="w-5 h-5"
