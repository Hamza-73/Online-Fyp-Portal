...
export default function StudentList({ currentUser }) {
    ...
    if (confirmDelete) {
      ...
      if (res.success) {
        ...
        setStudents((prevStudent) =>
          prevStudent.filter((students) => students._id !== id)
        );
      } else {
  ...
  const filteredStudents = students.filter((student) => {
    ...
    return (
      ...
      student.email?.toString().toLowerCase().includes(lowerQuery) ||
      student.rollNo?.toString().toLowerCase().includes(lowerQuery) ||
      student.batch?.toString().toLowerCase().includes(lowerQuery) ||
      student.semester?.toString().toLowerCase().includes(lowerQuery) ||
      student.department?.toString().toLowerCase().includes(lowerQuery) ||
      student.cnic?.toString().toLowerCase().includes(lowerQuery)
    );
  });
    ...
    <div className="container mx-auto px-4 py-8">
      ...
      <div className="container mx-auto px-4 py-8">
        ...
        <div className="mb-6 relative">
          ...
          <input
            type="text"
            placeholder="Search by name, CNIC, email, roll no, department, semester, batch, or father name"
            value={searchQuery}
            ...
            <form onSubmit={handleRegister}>
              <div className="grid grid-cols-2 gap-4">
                ...
                <div className="mb-4 relative">
                  ...
                  />
                  {errors.father && (
                    <p className="text-red-500 text-sm">{errors.father}</p>
                  )}
                </div>
                  ...
                  >
                    ...
                    {Array.from(
                      ...
                      (_, i) => {
                        const start = 2017 + i;
                        return (
                          <option key={start} value={`${start}-${start + 4}`}>
                            {`${start}-${start + 4}`}
                          </option>
                        );
                      }
