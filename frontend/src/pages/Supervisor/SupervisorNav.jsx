...
const SupervisorNav = ({ userData, onLogout, isSidebarOpen, toggleSidebar }) => {
  ...
  return (
    <header className="fixed top-0 left-0 w-full bg-maroon z-40">
      ...
      <div
        ref={sidebarRef}
        className={`fixed top-0 left-0 h-full bg-white shadow-md transition-transform duration-300 z-40 ${isSidebarOpen ? 'translate-x-0' : '-translate-x-full'
          }`}
        style={{ width: '250px' }}
      >
        <div className="flex flex-col p-4">
          <img src={LOGO} alt="GCU Logo" className="mb-4 w-20 mx-auto" />
          <Link to="/supervisor/announcements" className="p-2 text-lg hover:bg-gray-200">
            Announcements
          </Link>
          ...
          {(userData && userData.isCommittee) && <Link to='/supervisor/schedule-viva' className="p-2 text-lg hover:bg-gray-200">
            Add Deadline
          </Link>}
