...
import { FaUser, FaEnvelope, FaIdBadge, FaSchool, FaBook, FaIdCard } from 'react-icons/fa'; // Import relevant icons
import { toast, Toaster } from 'react-hot-toast';

export default function StudentProfile() {
  const { getProfile, editStudent } = useContext(StudentContext);
  ...
  const handleInputChange = (e) => {
    ...
    setIsSaveDisabled(!isValid || JSON.stringify(profile) === JSON.stringify({ ...formValues, [name]: value }));
  };

  const handleSave = async (e) => {
    e.preventDefault();
    const { name, rollNo, email, father, batch, semester, cnic, department } = formValues;

    if (!name || !rollNo || !email || !father || !batch || !semester || !cnic || !department) {
    ...
    if (response.success) {
      ...
      toast.error(response.message || 'An error occurred while updating the profile.');
    }
  };
