...
export default function MyGroup() {
    const { fetchMyGroup, uploadDocument, uploadProjectSubmission } = useContext(StudentContext);
    ...
    const handleTaskSubmission = async (e) => {
        ...
        if (file) formData.append("doc", file);
        if (webLink) formData.append("webLink", webLink);
        if (activeTask) formData.append("submissionType", activeTask.type.toLowerCase())
        setLoading(true);
    ...
    useEffect(() => {
        ...
        const { proposal, documentation, finalReport } = myGroup.deadlines.deadlines;
        const { proposal: proposalStatus, documentation: documentationStatus, finalReport: finalReportStatus } = myGroup.deadlines.status || {};
                    ...
                    <div className="max-w-9xl mx-auto bg-white p-8 rounded-lg shadow space-y-8 relative">
                        ...
                        {activeTask && (
                            <div className="absolute top-4 right-4 flex flex-col items-end space-y-2">
                                ...
                                {
                                    (!myGroup.submissions[activeTask.type.toLowerCase()].submitted && !myGroup.submissions[activeTask.type.toLowerCase()].documentLink) && <button button
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                            ...
                            <div className="border border-gray-300 p-6 rounded-lg shadow-sm">
                                ...
                                {myGroup.supervisor.designation && (
                                    <p className="text-lg">
                                        <strong>Designation:</strong> {myGroup.supervisor.designation}
                                    </p>
                                )}
                                        ...
                                        <div key={type} className="bg-gray-50 p-6 rounded-lg shadow flex flex-col md:flex-row items-center md:items-start">
                                            ...
                                            <div className="md:w-2/3 space-y-2">
                                                ...
                                                {myGroup.submissions[type].submitted ? (
                                                    <>
                                                        <p><strong>Submitted At:</strong> {new Date(myGroup.submissions[type].submittedAt).toLocaleString()}</p>
                                                        <p><strong>Submitted By:</strong> {`${myGroup.submissions[type].submittedBy.name} (${myGroup.submissions[type].submittedBy.rollNo})`}</p>
                                                        <p><strong>Document Link:</strong> <a href={myGroup.submissions[type].documentLink} target="_blank" rel="noopener noreferrer" className="text-blue-600 hover:underline">View Document</a></p>
                                                        <p><strong>Web Link:</strong> {myGroup.submissions[type].webLink ? <a href={myGroup.submissions[type].webLink} target="_blank" rel="noopener noreferrer" className="text-blue-600 hover:underline">{type} Link</a> : "N/A"}</p>
                                                    </>
                                                ) : (
                                            ...
                                            {myGroup.submissions[type].submitted && myGroup.submissions[type].documentLink && (
                        ...
                        >
                            ...
                            >
                                <h2 className="text-2xl font-semibold text-gray-900 text-center">
                                    {submissionModal ? `Upload ${activeTask.type}` : "Upload Document"}
                                </h2>

                                <form onSubmit={submissionModal ? handleTaskSubmission : handleUploadDocument} className="space-y-4">
                                    {/* File Upload */}
                                    ...
                                    />
                                    <p className="text-red-500 text-sm">Document must be in PDF format and under 2MB.</p>
                                    ...
                                    {/* Web Link Input */}
                                    <div>
                                        <label className="block text-gray-700 font-medium">Web Link (Optional)</label>
                                        <input
                                            type="text"
                                            ...
                                            className="w-full p-2 border border-gray-300 rounded-md focus:ring-2 focus:ring-blue-500 focus:outline-none"
                                        />
                                        {webLinkError && <p className="text-red-500 text-sm">{webLinkError}</p>}
                                    </div>
                                    ...
                                    <button
                                        ...
                                        className={`w-full py-3 rounded-md font-medium transition ${!file && !webLink
