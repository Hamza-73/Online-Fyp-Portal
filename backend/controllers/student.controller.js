...
module.exports.registerFromFile = async (req, res) => {
  ...
  try {
    ...
    for (let i = 0; i < excelData.length; i++) {
      ...
      if (!student.name || student.name.length < 3) {
        return res.status(400).json({
          success: false,
          message: `Name must be at least 3 characters for student at row ${
            i + 1
          }`,
        });
      }
      ...
      if (
        !student.rollNo ||
        !/^\d{4}(-RE-R)?(-R)?-BSCS-\d{2}$/.test(student.rollNo)
      ) {
        return res.status(400).json({
          success: false,
          message: `Invalid rollNo format for student at row ${i + 1}`,
        });
      }
      ...
      if (uniqueRollNos.has(student.rollNo)) {
        return res.status(400).json({
          success: false,
          message: `Duplicate rollNo found in row ${i + 1}: ${student.rollNo}`,
        });
      }
      ...
      if (!student.email || !emailRegex.test(student.email)) {
        return res.status(400).json({
          success: false,
          message: `Invalid email format for student at row ${i + 1}`,
        });
      }
      ...
      if (uniqueEmails.has(student.email)) {
        return res.status(400).json({
          success: false,
          message: `Duplicate email found in row ${i + 1}: ${student.email}`,
        });
      }
      ...
      if (!student.batch || !/^\d{4}-\d{4}$/.test(student.batch)) {
        return res.status(400).json({
          success: false,
          message: `Invalid batch format for student at row ${i + 1}`,
        });
      }
      ...
      if (!student.cnic || !/^\d{13}$/.test(student.cnic.toString())) {
        return res.status(400).json({
          success: false,
          message: `CNIC must be exactly 13 digits for student at row ${i + 1}`,
        });
      }
      ...
      if (isValid) {
        return res.status(400).json({
          success: false,
          message: `Student with email/rollNo already exists in the database at row ${
            i + 1
          }.`,
        });
      }
    ...
    return res.json({
      success: true,
      message: "File uploaded and students registered successfully.",
      newStudents,
    });
  } catch (error) {
...
module.exports.editStudentProfile = [
  ...
  async (req, res) => {
    ...
    try {
      ...
      const isAdmin =
        loggedInAdmin.superAdmin || loggedInAdmin.write_permission;

      // Only allow the owner to edit their email and password, or admin to edit any detail
      if (!isOwner && !isAdmin) {
        return res.status(403).json({
          message: "You do not have permission to edit this student's profile.",
        });
      }
...
module.exports.sendProjectRequest = async (req, res) => {
  ...
  try {
    ...
    if (student.isGroupMember) {
      return res
        .status(500)
        .json({ success: false, message: "You are already in a group" });
    }
    ...
    if (student.requests.pendingRequests.length >= 2) {
      return res.status(500).json({
        success: false,
        message: "You can only send request to 2 supervisors at a time",
      });
    }
    ...
    if (supervisor.slots <= 0)
      return res
        .status(500)
        .json({ success: false, message: "Supervisor slots are full" });

    // check if supervisor has already rejected request
    if (
      student.requests.rejectedRequests.some((req) =>
        req.equals(supervisor._id)
      )
    ) {
      return res.status(500).json({
        success: false,
        message:
          "You cannot send a request to this supervisor as he rejected your request before.",
      });
    }

    if (
      student.requests.pendingRequests.some((req) => req.equals(supervisor._id))
    ) {
      return res.status(400).json({
        success: false,
    ...
    if (existingProject) {
      return res.status(500).json({
        success: false,
        message: "Request with this project Title already exists",
      });
    }
    ...
    student.notifications.unseen.push({
      type: "Important",
      message: `Project request sent to ${supervisor.name}`,
    });
    ...
    supervisor.notifications.unseen.push({
      type: "Important",
      message: `A new proposal for ${projectTitle}`,
    });
...
module.exports.requestToJoinGroup = async (req, res) => {
  ...
  try {
    ...
    if (student.isGroupMember) {
      return res
        .status(400)
        .json({ success: false, message: "You're already in a Group" });
    }

    if (student.requests.pendingRequests.length >= 2) {
      return res.status(400).json({
        success: false,
        message: `You can only send to 2 supervisors at a time`,
      });
    }
    ...
    if (
      student.requests.rejectedRequests.some((request) =>
      ...
      return res.status(400).json({
        success: false,
        message:
          "You cannot send a request to this supervisor as they rejected your request before.",
      });
    }
    ...
    if (group.students.length >= 3) {
      return res
        .status(400)
        .json({ success: false, message: "The Group is already filled" });
    }

    // Check if the student has already sent a request to this supervisor
    if (
      student.requests.pendingRequests.some((request) =>
        request.equals(supervisor._id)
      )
    ) {
      return res.status(400).json({
        success: false,
        message: "You've already sent a request to this supervisor",
      });
    }
    ...
    res.json({
      success: true,
      message: `Request sent to ${supervisor.name} for ${group.title}`,
      notifications: student.notifications,
      requests: student.requests,
    });
  } catch (err) {
...
module.exports.myGroup = async (req, res) => {
  try {
    ...
    res.status(500).json({
      success: false,
      message: "Server error, try refreshing or logging in again",
    });
  }
};
...
module.exports.uploadDocument = async (req, res) => {
  try {
    ...
    if (req.files?.doc) {
      try {
        const result = await cloudinary.uploader.upload(
          req.files.doc.tempFilePath
        );
        docLink = result.secure_url; // Use `secure_url` for HTTPS link
      } catch (error) {
    ...
    if (!docLink && !webLink) {
      return res.status(400).json({
        success: false,
        message: "Either a document or web link must be provided",
      });
    }

    // Save document in group
    ...
    const notifyUsers = async (users, message) => {
      await Promise.all(
        users.map(async (stu) => {
          ...
          if (studentObj) {
            studentObj.notifications = studentObj.notifications || {
              seen: [],
              unseen: [],
            };
    ...
    const supervisor = await Supervisor.findById(group.supervisor);
    if (supervisor) {
      supervisor.notifications = supervisor.notifications || {
        seen: [],
        unseen: [],
      };
      supervisor.notifications.unseen.push({
...
module.exports.uploadProjectSubmission = async (req, res) => {
  try {
    ...
    const { proposal, documentation, project } =
      group.deadlines?.deadlines || {};
    const checkDeadline = (deadlineDate) => new Date(deadlineDate) < new Date();

    if (
      (submissionType.toLowerCase() === "proposal" &&
        proposal &&
        checkDeadline(proposal)) ||
      (submissionType.toLowerCase() === "documentation" &&
        documentation &&
        checkDeadline(documentation)) ||
      (submissionType.toLowerCase() === "project" &&
        project &&
    ...
    if (req.files?.doc) {
      try {
        ...
        // Notify students
        await Promise.all(
          group.students.map(async (stuId) => {
            ...
            if (studentObj) {
              ...
              studentObj.notifications.unseen.push({
                ...
                } has been uploaded by ${student.name}`,
              });
              await studentObj.save();
            }
          })
        ...
        if (supervisor) {
          ...
          supervisor.notifications.unseen.push({
            ...
            message: `${
              submissionType.charAt(0).toUpperCase() + submissionType.slice(1)
            } has been uploaded by ${student.name} for group: ${group.title}`,
          });
          await supervisor.save();
        }
