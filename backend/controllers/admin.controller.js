...
module.exports.register = [
    ...
    async (req, res) => {
        ...
        try {
            ...
            res.json({ success: true, message: 'Admin registration successful', newAdmin });
    ...
    try {
        ...
        for (let i = 0; i < excelData.length; i++) {
            ...
            if (uniqueEmails.has(user.email)) {
                return res.status(400).json({ success: false, message: `Duplicate email found in row ${i + 1}: ${user.email}` });
            }
            uniqueEmails.add(user.email);

            // Validate alphanumeric username
            if (user.username && !/^[a-zA-Z0-9]+$/.test(user.username)) {
                return res.status(400).json({ success: false, message: `Username should be alphanumeric for user at row ${i + 1}` });
            }

            // Check for duplicate username in the file
            if (user.username && uniqueUsernames.has(user.username.toLowerCase().replace(/\s+/g, ''))) {
                return res.status(400).json({ success: false, message: `Duplicate username found in row ${i + 1}: ${user.username}` });
            }
            uniqueUsernames.add(user.username);

            // Validate password length (at least 6 characters)
            if (!user.password || user.password.length < 6) {
                return res.status(400).json({ success: false, message: `Password must be at least 6 characters for user at row ${i + 1}` });
            }
            ...
            if (isValid) {
                return res.status(400).json({ success: false, message: `User with email/username already exists in the database at row ${i + 1}.` });
            }
...
module.exports.getAdmins = async (req, res) => {
    ...
    return res.json({ message: 'Admin Fteched successful', success: true, admin });
}
...
module.exports.editProfile = [
    ...
    body('fname').optional().isLength({ min: 3 }).withMessage('First name must be at least 3 characters'),
    body('lname').optional().isLength({ min: 3 }).withMessage('Last name must be at least 3 characters'),
    ...
    // Controller logic
    async (req, res) => {
        const { fname, lname, username, email, password, write_permission, superAdmin } = req.body;
        ...
        try {
            ...
            if (!isEditingOwnProfile && !isSuperAdmin && !loggedInAdmin.write_permission) {
                return res.status(403).json({ message: "You do not have permission to edit other admins." });
            }
            ...
            if (email || username) {
                const existingAdmin = await Admin.findOne({
                    $or: [
                        email ? { email } : {},
                        username ? { username: { $regex: new RegExp("^" + username.toLowerCase(), "i") } } : {}
                    ],
            ...
            if (isEditingOwnProfile) {
                ...
                if (isSuperAdmin) {
                    if (write_permission !== undefined) adminToEdit.write_permission = write_permission;
                ...
                if (write_permission !== undefined || superAdmin !== undefined) {
                    return res.status(403).json({ message: "You are not allowed to change write_permission or superAdmin status." });
                }
            } else {
            ...
            res.json({ success: true, message: 'Admin updated successfully', admin: adminToEdit });
...
module.exports.getProfile = async (req, res) => {
    try {
        ...
        const isOwnProfile = loggedInAdminId.toString() === adminData._id.toString();
        ...
        return res.status(403).json({ message: 'You do not have permission to view this profile', success: false });
